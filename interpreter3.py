...
if __name__ == "__main__":
    ...
    if errors:
        ...
        print("🎉 Workout successfully loaded!")
        print(f"💪 Workout Name: {workout['name']}")
        print(f"🎯 Goal: {workout['goal']}")
        print(f"📅 Scheduled Days: {', '.join(workout['schedule']) if workout['schedule'] else 'Not specified'}\n")

        print("📋 Exercises:")
        for i, ex in enumerate(workout["exercises"], start=1):
            if "name" in ex and "sets" in ex:
                print(f"  {i}. {ex['name']} - {ex['sets']} sets of {ex['reps']}")
            elif "name" in ex and "duration" in ex:
                print(f"  {i}. {ex['name']} - {ex['duration']} seconds")
            elif "rest" in ex:
                print(f"  {i}. rest for {ex['rest']} seconds")

        print("\n✅ You're all set. Crush your workout! 💥")